...
To edit a skill: open a PR against `Lullabot/lullabot-skills`. When it merges to `main`, the upstream `notify-prompt-library` workflow fires a `repository_dispatch` here, the `bump-skills` workflow updates the submodule pointer, and the site rebuilds — no manual bump needed.

To add a new skill: add a folder to `Lullabot/lullabot-skills` containing `SKILL.md` + `meta.yml` (and any resource files). Same auto-bump flow picks it up on merge.

To bump manually (e.g. while developing locally): `git submodule update --remote _skills-vendor && git add _skills-vendor`.
...
Skill submissions go to the upstream [Lullabot/lullabot-skills](https://github.com/Lullabot/lullabot-skills) repo, not this one.
